...
if [[ -z "$EXISTING_FW" ]]; then
  ...
  gcloud compute firewall-rules create "$FW_RULE" \
    ...
    --description="OpenClaw 電商+行銷+SEO 班 LINE Webhook + HTTPS" 2>/dev/null
...
echo ""
echo "  ⚠️  必須用下面這個指令（不可用 curl | sudo bash，會吞掉互動輸入）："
echo ""
echo "  curl -fsSL https://raw.githubusercontent.com/Joanna8521/openclaw-install_ecom/main/bootstrap.sh \\"
echo "    -o bootstrap.sh && chmod +x bootstrap.sh && sudo ./bootstrap.sh"
echo ""
echo "  ── ⚠️  費用提醒 ──────────────────────────────"
